...
class OurTeamController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->singular_title = 'Integrante';
        $this->action = 'our-team';
     ...
     * List all items
     *
     * @param OurTeamDataTable $dataTable
     * @return mixed
     */
    public function index(OurTeamDataTable $dataTable){
     ...
     *
     * @param StoreOurTeamRequest $request
     * @return JsonResponse
     */
    public function store(StoreOurTeamRequest $request){
        ...
        $path = $request->image->store('our-team');

        if(Storage::disk('local')->exists($path)){
            $file = Storage::disk('local')->get($path);
            $filename = basename($path);
            // dd($path,$filename, basename($path),  $request->input('image'));
            Storage::disk('our-team')->put($filename, $file);
     ...
     * @return JsonResponse
     */
    public function update(UpdateOurTeamRequest $request){

        canAccessTo($this->permissions->edit);

        $item = Team::find($request->input('id'));
    ...
    public function restore(Request $request){
        ...
        $item = Team::withTrashed()->find($request->input('id'));
