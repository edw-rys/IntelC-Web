            ...
            <form action="{{ route($route, $item->id) }}" id="formSave" method="PUT" enctype="multipart/form-data" onsubmit="saveData(event)">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group @error('work_amount') {{ 'is-invalid' }} @enderror">
                        ...
                        <input type="work_amount" class="form-control" name="work_amount" id="work_amount" value="{{ $item->work_amount }}">
                    ...
                    <div class="form-group @error('audit_amount') {{ 'is-invalid' }} @enderror">
                        ...
                        <input type="text" class="form-control" name="audit_amount" id="audit_amount" value="{{ $item->audit_amount }}">
                    ...
                    <div class="form-group @error('percentage') {{ 'is-invalid' }} @enderror">
                        ...
                        <div class="range-slider">
                            <input type="range" class="range-slider__range" name="percentage" id="percentage" min="0" max="100" value="{{ $item->percentage }}">
                            <span class="range-slider__value">{{ $item->percentage }}</span>
                        </div>
                    ...
                    <div class="form-group @error('type_id') {{ 'is-invalid' }} @enderror">
                        ...
                        <select name="type_id" id="type_id" class="select2 form-group" style="width: 100%">
                            @foreach ($types as $type)
                                <option value="{{ $type->id }}" {{ $item->type_id == $type->id ? 'selected':''}}>{{ $type->title }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group @error('type_id') {{ 'is-invalid' }} @enderror">
                        ...
                        <select name="icon" id="icon" class="select2_icon form-group" style="width: 100%">
                            @foreach ($icons as $icon)
                                <option value="{{ $icon->class }}" {{ $icon->class == $item->icon ? 'selected':''}}>{{ $icon->class }}</option>
                            @endforeach
                        </select>
                    </div>
